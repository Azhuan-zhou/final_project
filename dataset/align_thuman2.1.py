...
def main(args):
    body_model = SMPLX(model_path=SMPLX_PATH, gender='male', use_pca=True, num_pca_comps=45,flat_hand_mean=False, use_face_contour=True).to(device)
    for id in tqdm(range(10)):
        name_id = "%04d" % id
        # TODO: Note that you need to rename the THuman2.0 mesh and smplx folders
        input_file = os.path.join(args.input_path, 'THuman2.1', name_id, name_id + '.obj')
        tex_file = os.path.join(args.input_path, 'THuman2.1', name_id, 'material0.jpeg')
        smpl_file = os.path.join(args.input_path, 'THuman2.1_smplx', name_id, 'smplx_param.pkl')
        ...
        output = body_model(body_pose = torch.tensor(smpl_data['body_pose'], device=device,dtype=torch.float32),
                            betas = torch.tensor(smpl_data['betas'], device=device,dtype=torch.float32),
                            left_hand_pose = torch.tensor(smpl_data['left_hand_pose'], device=device,dtype=torch.float32),
                            right_hand_pose = torch.tensor(smpl_data['right_hand_pose'], device=device,dtype=torch.float32),
                            expression = torch.tensor(smpl_data['expression'], device=device,dtype=torch.float32),
                            jaw_pose = torch.tensor(smpl_data['jaw_pose'], device=device,dtype=torch.float32),
                            leye_pose = torch.tensor(smpl_data['leye_pose'], device=device,dtype=torch.float32),
                            reye_pose = torch.tensor(smpl_data['reye_pose'], device=device,dtype=torch.float32)
                                   )
